    ...
    for i in range(n_samples):
        for j in range(n_samples):
            if i != j:
                ...
                similarity_matrix[i, j] = 1  # The similarity of a vector with itself is 1.
    
    return similarity_matrix


def pearson_similarity2(vector, matrix):
    matrix = np.array(matrix)
    n_samples = matrix.shape[0]
    similarities = np.zeros(n_samples)
    
    for i in range(n_samples):
        similarities[i] = pearsonr(vector, np.array(matrix[i]))[0]
...
def my_get_l1_norms_filters_per_epoch(weight_list_per_epoch):
    ...
    for weight_array in weight_list_per_epoch:
        ...
        for epochs in weight_array:
            ...
            cosine_sim = pearson_similarity(flattened_filters)
...
def my_get_l1_norms_filters(model,first_time):
    ...
    for index, layer_index in enumerate(conv_layers):
        ...
        for i in range(num_filters):
            similarities = pearson_similarity2(filter_vectors[i], filter_vectors)[0]
